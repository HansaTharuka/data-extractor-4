...
#            break
    ...
    for ii in range(900):
        ...
        response = requests.get(src, stream=True)
        outfile = open('E:/Projects/Client/data-extractor-4/ExtractedData/Images/img{0}.jpg'.format(j), 'wb') #filename should be changed to increment gradually
        shutil.copyfileobj(response.raw, outfile)
        del response
        outfile.close()
...
#        desc = driver.find_element_by_xpath('//div/div/div[2]/div/div/div/div/div[3]/div/div/div').text
        ...
        desc_file = codecs.open("E:/Projects/Client/data-extractor-4/ExtractedData/PromodateCode/code{0}.txt".format(j), "w+", "utf16")
        desc_file.write(promocode)
        desc_file.close()
